...
%extend Prob {
	Prob(char const *uttid, int prob) {
		Prob *h = ckd_calloc(1, sizeof(*h));
		if (uttid)
...
%extend Decoder {
	...
	int decodeRaw(char const *filename) {
		FILE *fh = fopen(filename, "rb");
		if (!fh) return -1;
		
		int ret = ps_decode_raw($self, fh, NULL, -1);
	...
	Prob* getProb() {
		char const *uttid;
		int32 p;
		p = ps_get_prob($self, &uttid);
		return new_Prob(uttid, p);
	}
	~Decoder() {
		ps_free($self);
	}	
